...
for i, text_set in enumerate(st.session_state.text_sets):
    with st.sidebar.expander(f"Text Set {i + 1}", expanded=True):
        ...
        st.session_state.text_sets[i]["font_color"] = st.color_picker(f"Font Color {i + 1}", text_set["font_color"], key=f"font_color_{i}", disabled=disabled)
        st.session_state.text_sets[i]["font_stroke"] = st.slider(f"Font Stroke {i + 1}", 0, 10, st.session_state.text_sets[i]["font_stroke"], key=f"font_stroke_{i}", disabled=disabled)
        st.session_state.text_sets[i]["stroke_color"] = st.color_picker(f"Stroke Color {i + 1}", text_set["stroke_color"], key=f"stroke_color_{i}", disabled=disabled)
        st.session_state.text_sets[i]["text_opacity"] = st.slider(
            f"Text Opacity {i + 1}", 0.1, 1.0, st.session_state.text_sets[i]["text_opacity"], step=0.1, key=f"text_opacity_{i}", disabled=disabled
        )
        st.session_state.text_sets[i]["rotation"] = st.slider(f"Rotate Text {i + 1}", 0, 360, st.session_state.text_sets[i]["rotation"], key=f"rotation_{i}", disabled=disabled)
