...
for i, text_set in enumerate(st.session_state.text_sets):
    with st.sidebar.expander(f"Text Set {i + 1}", expanded=True):
        ...
        st.session_state.text_sets[i]["font_size"] = st.number_input(f"Font Size {i + 1}", min_value=10, max_value=800, value=text_set["font_size"], key=f"font_size_{i}", disabled=disabled)
        ...
        st.session_state.text_sets[i]["x_position"] = st.number_input(f"X Position {i + 1}", min_value=-800, max_value=800, value=text_set["x_position"], key=f"x_position_{i}", disabled=disabled)
        st.session_state.text_sets[i]["y_position"] = st.number_input(f"Y Position {i + 1}", min_value=-800, max_value=800, value=text_set["y_position"], key=f"y_position_{i}", disabled=disabled)
